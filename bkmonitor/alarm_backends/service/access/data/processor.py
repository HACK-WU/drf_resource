...
class AccessDataProcess(BaseAccessDataProcess):
    ...
    def query_data(self, now_timestamp: int) -> list[dict]:
        ...
        try:
            ...
            logger.exception(
                f"strategy_group_key({self.strategy_group_key}) query records error, {e}"
            )
            points = []
    ...
    def push(self, records: list = None, output_client=None):
        ...
        if not self.sub_task_id:
            logger.info(
                f"strategy_group_key({self.strategy_group_key}), push records({len(self.record_list)}), last_checkpoint({arrow.get(last_checkpoint).strftime(constants.STD_LOG_DT_FORMAT)})"
            )
        else:
