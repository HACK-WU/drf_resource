...
class TestSimpleRingRatio:
    def test_detect_floor(self):
        with mock.patch(
            ...
            detect_engine = SimpleRingRatio(config=algorithms_config)
    ...
    def test_anomaly_message(self):
        with mock.patch(
            ...
            assert anomaly_result[0].anomaly_message == "avg(测试指标)较前一时刻(99%)上升超过100.0%, 当前值200%"
            anomaly_result = detect_engine.detect_records(_datapoint0, 1)
            assert len(anomaly_result) == 1
            assert anomaly_result[0].anomaly_message == "avg(测试指标)较前一时刻(99%)下降超过50.0%, 当前值0%"
    ...
    def test_invalid_config(self):
        ...
        with pytest.raises(InvalidSimpleRingRatioConfig):
            detect_engine = SimpleRingRatio(config=algorithms_config)
            detect_engine.detect((99, 100000000))
