...
def add_bkunifylogbeat_common_datasource(apps, *args, **kwargs):
    ...
    data_id = 1100015
    # data_id对应的名称
    data_name = f"{settings.AGGREGATION_BIZ_ID}_bkunifylogbeat_k8s_common"
    # 结果表id
    table_id = "bkunifylogbeat_k8s_common.base"
    # 结果表名称
    table_name_zh = "bkunifylogbeat K8S 公共指标"
    # 数据清洗类型
    ...
    schema_type = "free"
    # 实际influxdb对应的数据库名
    database = "bkunifylogbeat_k8s_common"
    # 实际influxdb对应的表名
    ...
    models["TimeSeriesGroup"].objects.create(
        ...
        is_enable=True,
        table_id=table_id,
        time_series_group_name="bkunifylogbeat k8s common metrics",
    )
...
def add_bkunifylogbeat_task_datasource(apps, *args, **kwargs):
    ...
    data_id = 1100016
    # data_id对应的名称
    data_name = f"{settings.AGGREGATION_BIZ_ID}_bkunifylogbeat_k8s_task"
    # 结果表id
    table_id = "bkunifylogbeat_k8s_task.base"
    # 结果表名称
    table_name_zh = "bkunifylogbeat k8s 任务指标"
    # 数据清洗类型
    etl_config = "bk_standard_v2_time_series"
    # 来源标签
    source_label = "bk_monitor"
    ...
    schema_type = "free"
    # 实际influxdb对应的数据库名
    database = "bkunifylogbeat_k8s_task"
    # 实际influxdb对应的表名
    real_table_name = "base"
    # rp过期时间
    source_duration_time = "30d"
    ...
    models["TimeSeriesGroup"].objects.create(
        ...
        is_enable=True,
        table_id=table_id,
        time_series_group_name="bkunifylogbeat k8s task metrics",
    )
