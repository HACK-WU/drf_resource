...
def add_operation_data_custom_series(apps, *args, **kwargs):
    ...
    data_id = 1100012
    # data_id对应的名称
    data_name = f"{settings.AGGREGATION_BIZ_ID}_operation_data_custom_series"
    # 结果表id
    table_id = "operation_data_custom_series.base"
    # 结果表名称
    table_name_zh = "运营数据自定义上报时序数据"
    # 数据清洗类型
    ...
    schema_type = "free"
    # 实际influxdb对应的数据库名
    database = "operation_data_custom_series"
    # 实际influxdb对应的表名
    ...
    models["TimeSeriesGroup"].objects.create(
        ...
        is_enable=True,
        table_id=table_id,
        time_series_group_name="operation_data_custom_series",
    )
...
class Migration(migrations.Migration):
    ...
    operations = [
        migrations.RunPython(add_operation_data_custom_series),
    ]
