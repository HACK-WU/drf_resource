...
DEVX_TABLE_ID_PREFIX = "devx"


@pytest.fixture
def clean_records():
    yield
    models.DataSourceResultTable.objects.filter(table_id__startswith=DEVX_TABLE_ID_PREFIX).delete()
    models.ResultTable.objects.filter(table_id__startswith=DEVX_TABLE_ID_PREFIX).delete()
    models.ResultTableField.objects.filter(table_id__startswith=DEVX_TABLE_ID_PREFIX).delete()
    models.ResultTableFieldOption.objects.filter(table_id__startswith=DEVX_TABLE_ID_PREFIX).delete()
    models.ResultTableOption.objects.filter(table_id__startswith=DEVX_TABLE_ID_PREFIX).delete()
    models.InfluxDBStorage.objects.filter(table_id__startswith=DEVX_TABLE_ID_PREFIX).delete()
    models.SpaceDataSource.objects.filter(space_type_id="bkci").delete()


def test_add_bkci_system_dimensions(create_and_delete_record, mocker, clean_records):
    ...
    # 判断 option 存在
    check_option_name = ["enable_devx_meta", "must_include_dimensions", "mapping_result_table"]
