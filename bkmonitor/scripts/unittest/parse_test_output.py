...
def handle_testcase(output: str) -> dict[str, int]:
    ...
    category_mapping: dict[str, str] = {"failures": "failed"}
    test_result: dict[str, int] = {}
...
def handle_coverage(output: str) -> dict[str, str | int]:
    pattern = re.compile(r"TOTAL\s+(\d+)\s+(\d+)\s+(\d+%)")
    coverage_result_tuple: tuple[str, str, str] = pattern.findall(output)[0]
    coverage_result: dict[str, str | int] = {
