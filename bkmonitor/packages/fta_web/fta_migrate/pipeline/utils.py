...
from fta_web.fta_migrate.pipeline.tree_components import line_uniqid, node_uniqid
...
def _replace_flow_in_node(node_id, pipeline_data, substituted_id, flow_id, field):
    if node_id in pipeline_data[PE.activities]:
        ...
        if node[PE.type] == PE.ExclusiveGateway and field == PE.outgoing:
            replace_flow_in_exclusive_gateway(node, substituted_id, flow_id)
...
def replace_flow_in_exclusive_gateway(gateway, substituted_id, flow_id):
