...
from bkmonitor.dataflow.node.processor import DownsamplingNode
...
class StatisticTask(BaseTask):
    ...
    1. 先创建Flow，得到data_flow对象
    2. 再创建node，在data_flow对象身上通过add方法添加node
    3. DataFlow和Node只是一堆参数的集合
    """
    ...
    @property
    def flow_name(self):
        return "{} {}".format(_("统计任务"), self.rt_id)
