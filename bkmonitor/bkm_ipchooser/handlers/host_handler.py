    ...
    def details(
        ...
        for host_info in host_list:
            if "host_id" in host_info:
                host_ids.append(host_info["host_id"])
            else:
                ips.append(host_info["ip"])
                ip_keys.append((host_info["ip"], host_info["cloud_id"]))

        if host_ids:
            rules.append({"field": "bk_host_id", "operator": "in", "value": host_ids})
        # 使用in语法批量查询主机ip，优化查询条件，后续使用云区域id对查询出的主机信息进行过滤
        if ips:
            rules.append({"field": "bk_host_innerip", "operator": "in", "value": ips})

        if rules:
            host_property_filter = {"condition": "OR", "rules": rules}
        else:
            host_property_filter = {}

        origin_hosts = cls.details_base(scope_list, host_property_filter)

        hosts = []
        for host in origin_hosts:
            if host["host_id"] in host_ids or (host["ip"], host["cloud_id"]) in ip_keys:
                hosts.append(host)
        return hosts
