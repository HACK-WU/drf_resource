...
class PaginationSer(serializers.Serializer):
    start = serializers.IntegerField(help_text="数据起始位置", required=False, default=0)
    page_size = serializers.IntegerField(
        help_text="拉取数据数量，不传或传 `-1` 表示拉取所有",
        required=False,
...
class ScopeSer(serializers.Serializer):
    scope_type = serializers.ChoiceField(help_text="资源范围类型", choices=constants.ScopeType.list_choices())
    scope_id = serializers.CharField(help_text="资源范围ID", min_length=1)
    # 最终只会使用 bk_biz_id
    bk_biz_id = serializers.IntegerField(help_text="业务 ID", required=False)
...
class TreeNodeSer(serializers.Serializer):
    object_id = serializers.CharField(help_text="节点类型ID")
    instance_id = serializers.IntegerField(help_text="节点实例ID")
    meta = ScopeSer(help_text="Meta元数据", required=False)


class HostSearchConditionSer(serializers.Serializer):
    ip = serializers.IPAddressField(label="内网IP", required=False, protocol="ipv4")
    ipv6 = serializers.IPAddressField(label="内网IPv6", required=False, protocol="ipv6")
    os_type = serializers.ChoiceField(label="操作系统类型", required=False, choices=constants.OS_CHOICES)
    host_name = serializers.CharField(label="主机名称", required=False, min_length=1)
    content = serializers.CharField(label="模糊搜索内容（支持同时对`主机IP`/`主机名`/`操作系统`进行模糊搜索", required=False, min_length=1)


class ScopeSelectorBaseSer(serializers.Serializer):
    all_scope = serializers.BooleanField(help_text="是否获取所有资源范围的拓扑结构，默认为 `false`", required=False, default=False)
    scope_list = serializers.ListField(help_text="要获取拓扑结构的资源范围数组", child=ScopeSer(), default=[], required=False)
...
class QueryHostsBaseSer(ScopeSelectorBaseSer, PaginationSer):
    ...
    # 适配原代码风格
    conditions = serializers.ListField(label="搜索条件", required=False, child=serializers.DictField())
    ...
    def validate(self, attrs):
        ...
        if "search_content" in attrs:
            if is_v6(attrs["search_content"]):
                attrs["search_content"] = exploded_ip(attrs["search_content"])
...
class HostInfoWithMetaSer(serializers.Serializer):
    meta = ScopeSer(help_text="Meta元数据", required=False)
    cloud_id = serializers.IntegerField(help_text="云区域 ID", required=False)
    ip = serializers.IPAddressField(help_text="IPv4 协议下的主机IP", required=False, protocol="ipv4")
    host_id = serializers.IntegerField(help_text="主机 ID，优先取 `host_id`，否则取 `ip` + `cloud_id`", required=False)
