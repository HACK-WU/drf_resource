...
class HostCheckRequestSer(base.ScopeSelectorBaseSer):
    ip_list = serializers.ListField(
        help_text="IPv4 列表",
        child=serializers.CharField(help_text="IPv4，支持的输入格式：`cloud_id:ip` / `ip`", min_length=1),
        default=[],
        required=False,
    )
    ipv6_list = serializers.ListField(
        help_text="IPv6 列表",
        child=serializers.CharField(help_text="IPv6，支持的输入格式：`cloud_id:ipv6` / `ipv6`", min_length=1),
        default=[],
        required=False,
    )
    key_list = serializers.ListField(
        help_text="关键字列表",
        child=serializers.CharField(help_text="关键字，解析出的`主机名`、`host_id` 等关键字信息", min_length=1),
        default=[],
        required=False,
    )
