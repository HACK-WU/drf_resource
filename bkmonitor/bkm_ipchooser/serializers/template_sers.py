...
class BaseTemplateSer(base.ScopeSelectorBaseSer):
    ...
    template_type = serializers.ChoiceField(
        help_text="模板类型", required=True, choices=constants.TemplateType.list_choices()
    )
...
class ListTemplateRequestSer(serializers.Serializer):
    ...
    id = serializers.IntegerField(help_text="模板ID", required=True)
    meta = base.ScopeSer(help_text="Meta元数据", required=False)
...
class ServiceInstanceCountSer(AgentStatisticsSer):
    """模板服务实例统计"""
