...
from bkm_ipchooser.serializers import (
    config_sers,
    dynamic_group_sers,
    host_sers,
    template_sers,
...
class IpChooserTopoViewSet(CommonViewSet):
    ...
    def trees(self, request, *args, **kwargs):
        return Response(
            topo_handler.TopoHandler.trees(
                ...
                count_instance_type=self.validated_data["count_instance_type"],
            )
        )
    ...
    def query_path(self, request, *args, **kwargs):
        return Response(
            topo_handler.TopoHandler.query_path(
                scope_list=self.validated_data["scope_list"],
                node_list=self.validated_data["node_list"],
                count_instance_type=self.validated_data["count_instance_type"],
            )
        )
    ...
    def query_service_instances(self, request, *args, **kwargs):
        return Response(
            topo_handler.TopoHandler.query_service_instance(
                scope_list=self.validated_data["scope_list"],
                readable_node_list=self.validated_data["node_list"],
                service_instance_ids=self.validated_data.get("service_instance_id_list"),
                search_content=self.validated_data.get("search_content"),
                start=self.validated_data["start"],
                page_size=self.validated_data["page_size"],
            )
        )
    ...
    @list_route(methods=["POST"], serializer_class=topo_sers.ServiceInstanceCountRequestSer)
    def service_instance_count(self, request, *args, **kwargs):
        return Response(
            topo_handler.TopoHandler.node_service_instances(
                scope_list=self.validated_data["scope_list"], node_list=self.validated_data["node_list"]
            )
        )
...
class IpChooserTemplateViewSet(CommonViewSet):
    ...
    def service_instance_count(self, request, *args, **kwargs):
        return Response(
            template_handler.TemplateHandler(
                scope_list=self.validated_data["scope_list"], template_type=self.validated_data["template_type"]
            ).service_instance_counts(template_id_list=self.validated_data["template_id_list"])
        )
