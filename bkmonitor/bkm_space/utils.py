...
def bk_biz_id_to_space_uid(bk_biz_id: str | int) -> str:
    ...
    space = api.SpaceApi.get_space_detail(bk_biz_id=bk_biz_id)
...
def is_bk_ci_space(space_uid: str) -> bool:
    """判断是否是bk ci类型的空间 ."""
    if not space_uid:
        return False
    space_type, _ = parse_space_uid(space_uid)
    return space_type == SpaceTypeEnum.BKCI.value


def is_bk_saas_space(space_uid: str) -> bool:
    if not space_uid:
        return False
    space_type, _ = parse_space_uid(space_uid)
    return space_type == SpaceTypeEnum.BKSAAS.value
