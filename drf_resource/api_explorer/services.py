...
class APIDiscoveryService:
    ...
    @classmethod
    def _extract_module_apis(cls, module_name: str, module_obj: APIResourceShortcut, search: Optional[str]) -> List[Dict]:
        """
        提取模块下的所有 API
    ...
    def _match_search(cls, metadata: Dict, search: str) -> bool:
        ...
        # 匹配模块名、接口名、类名、标签
        return (
            search_lower in metadata.get('module', '').lower() or
            search_lower in metadata.get('name', '').lower() or
            search_lower in metadata.get('class_name', '').lower() or
            search_lower in metadata.get('label', '').lower()
        )
