...
try:
    ...
    class BKAPIError(Exception):
        def __init__(self, system_name=None, url=None, result=None):
...
try:
    ...
    class APIPermissionDeniedError(Exception):
        def __init__(self, context=None, data=None, extra=None):
            ...
            self.data = data
            self.extra = extra
            super().__init__("API Permission Denied")
...
try:
    ...
    class MockMetrics:
        class API_FAILED_REQUESTS_TOTAL:
            @staticmethod
            def labels(**kwargs):
                return MockMetrics.MockCounter()
...
class APIResource(six.with_metaclass(abc.ABCMeta, DocumentationMixin, CacheResource)):
    """
    API类型的Resource
    """
