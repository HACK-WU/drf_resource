...
class BKAPIError(ExternalServiceException):
    """
    蓝鲸 API 调用异常

    专门用于蓝鲸平台 API 调用失败时抛出的异常。
    继承自 ExternalServiceException，提供与旧 APIError 兼容的接口。
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # 从 result 中提取消息
        response_code = self._result.get("code")
        response_message = self._result.get("message", "API request failed")
    ...
    def to_dict(self) -> dict:
        """转换为字典格式，包含 API 响应结果"""
        data = super().to_dict()
        data["error"]["result"] = self._result
        return data
