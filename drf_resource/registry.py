...
class ResourceRegistry:
    ...
    def register_resource(self, name: str, resource_class: Type, module_path: str = None, 
                         custom_name: bool = False) -> None:
        """
        注册资源类
    ...
    def register_function(self, name: str, function: callable, module_path: str = None,
                         custom_name: bool = False) -> None:
        """
        注册函数资源
    ...
    def sync_to_manager(self) -> None:
        ...
        保持与现有调用方式的兼容性
        """
        from drf_resource.management.root import resource, api, adapter
    ...
    def _infer_module_path(self, resource_class: Type) -> str:
        ...
        # 移除常见的资源模块后缀
        if module_name.endswith('.resources'):
            return module_name[:-10]  # 移除 '.resources'
        elif module_name.endswith('.default'):
            return module_name[:-8]   # 移除 '.default'
        elif module_name.endswith('.api.resources'):
            return module_name[:-14] + '.api'  # 转换为 api 路径
        elif module_name.endswith('.adapter.default'):
            return module_name[:-15]  # 移除 '.adapter.default'
    ...
    def _create_resource_shortcut(self, module_path: str, resources: Dict[str, Dict[str, Any]]) -> None:
        ...
        from drf_resource.management.root import (
            ResourceShortcut, APIResourceShortcut, AdapterResourceShortcut,
            resource, api, adapter, is_api, is_adapter
        )
        
        # 创建虚拟的 ResourceShortcut 实例
        class DynamicResourceShortcut:
            def __init__(self, module_path: str, resources: Dict[str, Dict[str, Any]]):
                self._path = module_path + '.resources'
                self._methods = {}
                self.loaded = True
...
class ResourceMeta(abc.ABCMeta):
    ...
    def __new__(mcs, name, bases, namespace, **kwargs):
        ...
        if not any(issubclass(base, Resource) for base in bases if base != object):
